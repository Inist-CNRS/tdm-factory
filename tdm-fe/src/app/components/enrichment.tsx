...
import { DefaultApiFactory, Request } from '@/generated/api';
...
const Enrichment: React.FC<EnrichmentProps> = (props: EnrichmentProps) => {
  ...
  const handleChangeEnrichment = (event: any) => {
    props.selectEnrichment(dataEnrichments.find((enrichment: Request) => enrichment.label === event.target.textContent) || {});
  ...
  return (
    <React.Fragment>
      <MuiFileInput error={props.fileError} content="title" label="Déposer un fichier" value={file} onChange={handleFileInput} fullWidth sx={{ mb: 2, mt: 2, minWidth: 200 }} />
      {props.fileError ? (<p className="error">Le fichier n&apos;est pas compatible avec le convertisseur, utilisez le type {selectedWrapper?.fileType}.</p>) : null}
      ...
      {selectedWrapper ? (
        <ReactMarkdown>{selectedWrapper.description}</ReactMarkdown>
      ) : null}
      {selectedWrapper?.parameters !== undefined && selectedWrapper.parameters.length > 0 ? (
        <Box sx={{ p: 2, border: '1px' }}>
          {selectedWrapper?.parameters?.map((param: any) => (
            <TextField key={param.label} sx={{ m: 2, minWidth: 200 }} id={param.label} label={param.displayName} variant="outlined" fullWidth
      ...
      {selectedEnrichment ? (
        <ReactMarkdown>{selectedEnrichment.description}</ReactMarkdown>
      ) : null}
    </React.Fragment>
  );
};
