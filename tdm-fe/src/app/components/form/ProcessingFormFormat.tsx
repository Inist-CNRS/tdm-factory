...
const ProcessingFormFormat = ({ onChange, value }: ProcessingFormFormatProps) => {
    ...
    const handleFormatClick = useCallback((format: string, event: React.MouseEvent) => {
                    ...
                    >
                        {Object.entries(formatDetails).map(([format, details]) => (
                            <div 
                                ...
                                onClick={(e) => handleFormatClick(format, e as React.MouseEvent)}
                            >
                                <div className="format-label-container">
                                    <FormControlLabel
                                        value={format}
                                        control={<Radio />}
                                        label={format === 'istex' ? 'Corpus Istex (tar.gz)' :
                                               format === 'text' ? 'Corpus de documents textes (tar.gz)' :
                                               'Tableur (.csv)'}
                                    />
