    ...
    return Object.entries(res.data.paths)
      ...
      .map<Request>((path: any[]) => {
        ...
        return {
          ...
          parameters: path[1].post?.parameters?.map((param: any) => { return { name: param.name, displayName: param.description + ` (par défaut ${param.name})` } }).filter((param: Parameter) => param.name === 'value'),
        };
      })
  });
  res.json(requests);
